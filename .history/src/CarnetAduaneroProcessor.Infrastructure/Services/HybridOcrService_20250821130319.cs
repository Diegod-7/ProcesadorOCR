...
namespace CarnetAduaneroProcessor.Infrastructure.Services
{
    ...
    public class HybridOcrService : IHybridOcrService
    {
        ...
        private void InitializeTesseract()
        {
            try
            {
                ...
                if (Directory.Exists(Path.Combine(_tessdataPath, "spa.traineddata")))
                {
                    _tesseractEngine = new TesseractEngine(_tessdataPath, "spa", EngineMode.Default);
                    _logger.LogInformation("Tesseract inicializado con idioma español");
                }
                else if (Directory.Exists(Path.Combine(_tessdataPath, "eng.traineddata")))
                {
                    _tesseractEngine = new TesseractEngine(_tessdataPath, "eng", EngineMode.Default);
                    _logger.LogInformation("Tesseract inicializado con idioma inglés");
                }
                else
                {
            ...
            {
                _logger.LogError(ex, "Error inicializando Tesseract. Usando Azure Computer Vision como fallback.");
                _tesseractEngine = null;
            }
        }
        ...
        public async Task<string> ExtractTextAsync(Bitmap image)
        {
            try
            {
                ...
                {
                    var tesseractResult = await ExtractTextWithTesseractAsync(image);
                ...
                if (!string.IsNullOrEmpty(_azureVisionKey) && !string.IsNullOrEmpty(_azureVisionEndpoint))
                {
                    ...
                    return await ExtractTextWithAzureVisionAsync(image);
                }
    ...
    /// <summary>
    /// Interfaz para el servicio híbrido de OCR
    /// </summary>
