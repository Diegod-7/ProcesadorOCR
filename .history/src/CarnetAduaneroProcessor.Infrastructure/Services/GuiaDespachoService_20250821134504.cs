...
namespace CarnetAduaneroProcessor.Infrastructure.Services
{
    ...
    public class GuiaDespachoService : IGuiaDespachoService
    {
        ...
        public GuiaDespachoService(ILogger<GuiaDespachoService> logger, IConfiguration configuration, IHybridOcrService hybridOcrService)
        {
            _logger = logger;
        ...
        public async Task<GuiaDespacho> ProcesarTextoOcrAsync(string textoOcr)
        {
            ...
            try
            {
                ...
                var esFormatoJorgeStein = textoNormalizado.Contains("Jorge Stein") || textoNormalizado.Contains("stein.cl");
                var esFormatoAlbertoRubio = textoNormalizado.Contains("Alberto Rubio") || textoNormalizado.Contains("agenciarubio.cl");
                var esFormatoRicardoMewes = textoNormalizado.Contains("RICARDO MEWES") || textoNormalizado.Contains("MEWES SCHNAIDT");
                var esFormatoAlexisMontenegro = textoNormalizado.Contains("ALEXIS MONTENEGRO") || textoNormalizado.Contains("AGENCIA DE ADUANA Y ASESORIAS ELECTRONICA");
                ...
                await ExtraerCamposCriticosAsync(documento, textoNormalizado, esFormatoJorgeStein, esFormatoAlbertoRubio, esFormatoRicardoMewes, esFormatoAlexisMontenegro);

                // Extraer campos adicionales
                await ExtraerCamposAdicionalesAsync(documento, textoNormalizado, esFormatoJorgeStein, esFormatoAlbertoRubio, esFormatoRicardoMewes, esFormatoAlexisMontenegro);

                // Búsqueda final de campos críticos que no se pudieron extraer
                BusquedaFinalCamposCriticos(documento, textoNormalizado, esFormatoJorgeStein, esFormatoAlbertoRubio, esFormatoRicardoMewes, esFormatoAlexisMontenegro);
        ...
        private async Task ExtraerCamposCriticosAsync(GuiaDespacho documento, string texto, bool esFormatoJorgeStein, bool esFormatoAlbertoRubio, bool esFormatoRicardoMewes, bool esFormatoAlexisMontenegro)
        {
            ...
            else if (esFormatoRicardoMewes)
            {
                // Formato Ricardo Mewes: Nº 27499
                var matchGuia = Regex.Match(texto, @"N[°º]\s*(\d+)", RegexOptions.IgnoreCase);
                if (matchGuia.Success)
                {
                    numeroGuia = matchGuia.Groups[1].Value;
                    _logger.LogInformation("Número de guía (Ricardo Mewes) extraído: {Guia}", numeroGuia);
                }
            }
            else if (esFormatoAlexisMontenegro)
            {
                // Formato Alexis Montenegro: Nº44172
                var matchGuia = Regex.Match(texto, @"N[°º](\d+)", RegexOptions.IgnoreCase);
                if (matchGuia.Success)
                {
                    numeroGuia = matchGuia.Groups[1].Value;
                    _logger.LogInformation("Número de guía (Alexis Montenegro) extraído: {Guia}", numeroGuia);
                }
            }
            else
            {
                // Intentar múltiples formatos como fallback
                var matchGuia = Regex.Match(texto, @"(?:No\s+|N[°º]\s*|N[°º])(\d+)", RegexOptions.IgnoreCase);
                if (matchGuia.Success)
                {
                    numeroGuia = matchGuia.Groups[1].Value;
            ...
            {
                // Formato Ricardo Mewes: Rut: : 76.627.544-3 (con doble punto)
                var matchRutEmisor = Regex.Match(texto, @"AG\.\s+DE\s+ADUANAS.*?Rut:\s*:\s*(\d{1,2}\.\d{3}\.\d{3}-\d)", RegexOptions.IgnoreCase | RegexOptions.Singleline);
                if (matchRutEmisor.Success)
                {
                    rutEmisor = matchRutEmisor.Groups[1].Value;
                    _logger.LogInformation("RUT del emisor (Ricardo Mewes) extraído: {Rut}", rutEmisor);
                }
                else
                {
                    // Patrón alternativo para capturar Rut: : (con doble punto)
                    var matchRutEmisorAlt = Regex.Match(texto, @"Rut:\s*:\s*(\d{1,2}\.\d{3}\.\d{3}-\d)", RegexOptions.IgnoreCase);
                    if (matchRutEmisorAlt.Success)
                    {
                        rutEmisor = matchRutEmisorAlt.Groups[1].Value;
                        _logger.LogInformation("RUT del emisor (Ricardo Mewes alt) extraído: {Rut}", rutEmisor);
                    }
                }
            }
            else if (esFormatoAlexisMontenegro)
            {
                // Formato Alexis Montenegro: R.U.T .: 13.021.175-5
                var matchRutEmisor = Regex.Match(texto, @"R\.U\.T\s*\.\s*:\s*(\d{1,2}\.\d{3}\.\d{3}-\d)", RegexOptions.IgnoreCase);
                if (matchRutEmisor.Success)
                {
                    rutEmisor = matchRutEmisor.Groups[1].Value;
                    _logger.LogInformation("RUT del emisor (Alexis Montenegro) extraído: {Rut}", rutEmisor);
                }
            }
            else
            {
                // Intentar múltiples formatos como fallback incluyendo doble punto
                var matchRutEmisor = Regex.Match(texto, @"(?:R\.U\.T\s*\.?\s*:|Rut:\s*:)\s*(\d{1,2}\.\d{3}\.\d{3}-\d)", RegexOptions.IgnoreCase);
                if (matchRutEmisor.Success)
                {
                    rutEmisor = matchRutEmisor.Groups[1].Value;
                ...
                {
                    // Patrón más simple para capturar cualquier RUT al inicio del documento
                    var matchRutSimple = Regex.Match(texto, @"(\d{1,2}\.\d{3}\.\d{3}-\d)", RegexOptions.IgnoreCase);
                    if (matchRutSimple.Success)
                    {
                        rutEmisor = matchRutSimple.Groups[1].Value;
                        _logger.LogInformation("RUT del emisor (simple) extraído: {Rut}", rutEmisor);
                    }
                }
            }
            ...
            {
                // Formato Ricardo Mewes: FECHA: 24-06-2025
                var matchFecha = Regex.Match(texto, @"FECHA:\s*(\d{1,2}-\d{1,2}-\d{4})", RegexOptions.IgnoreCase);
                if (matchFecha.Success)
                {
                    var fechaStr = matchFecha.Groups[1].Value;
                    if (DateTime.TryParseExact(fechaStr, "dd-MM-yyyy", null, System.Globalization.DateTimeStyles.None, out var fecha))
                    {
                        fechaDocumento = fecha;
                        _logger.LogInformation("Fecha del documento (Ricardo Mewes) extraída: {Fecha}", fechaDocumento);
                    }
                }
            }
            else if (esFormatoAlexisMontenegro)
            {
                // Formato Alexis Montenegro: buscar fecha en el texto (puede estar en diferentes formatos)
                var matchFecha = Regex.Match(texto, @"Fecha\s+(\d{1,2}-\d{1,2}-\d{4})", RegexOptions.IgnoreCase);
                if (!matchFecha.Success)
                {
                ...
                if (matchFecha.Success)
                {
                    ...
                    {
                        fechaDocumento = fecha;
                        _logger.LogInformation("Fecha del documento (Alexis Montenegro) extraída: {Fecha}", fechaDocumento);
                    }
                }
            }
            else
        ...
        private async Task ExtraerCamposAdicionalesAsync(GuiaDespacho documento, string texto, bool esFormatoJorgeStein, bool esFormatoAlbertoRubio, bool esFormatoRicardoMewes, bool esFormatoAlexisMontenegro)
        {
            ...
            {
                // Formato Ricardo Mewes: AG. DE ADUANAS RICARDO MEWES SCHNAIDT Y CIA LTDA.
                var matchNombreEmisor = Regex.Match(texto, @"(AG\.\s+DE\s+ADUANAS\s+[A-ZÁÉÍÓÚÑ\s]+?Y\s+CIA\s+LTDA\.)", RegexOptions.IgnoreCase);
                if (matchNombreEmisor.Success)
                {
                    nombreEmisor = matchNombreEmisor.Groups[1].Value.Trim();
                    _logger.LogInformation("Nombre del emisor (Ricardo Mewes) extraído: {Nombre}", nombreEmisor);
                }
            }
            else if (esFormatoAlexisMontenegro)
            {
                // Formato Alexis Montenegro: ALEXIS MONTENEGRO PONCE
                var matchNombreEmisor = Regex.Match(texto, @"R\.U\.T\s*\.\s*:\s*\d{1,2}\.\d{3}\.\d{3}-\d\s+([A-ZÁÉÍÓÚÑ\s]+?)\s+GUIA\s+DE\s+DESPACHO", RegexOptions.IgnoreCase);
                if (matchNombreEmisor.Success)
                {
                    nombreEmisor = matchNombreEmisor.Groups[1].Value.Trim();
                    _logger.LogInformation("Nombre del emisor (Alexis Montenegro) extraído: {Nombre}", nombreEmisor);
                }
            }
            ...
            {
                // Fallback: buscar cualquier texto que contenga "ADUANAS" y "CIA"
                var matchNombreEmisorFallback = Regex.Match(texto, @"([A-ZÁÉÍÓÚÑ\s\.]*ADUANAS[A-ZÁÉÍÓÚÑ\s\.]*CIA[A-ZÁÉÍÓÚÑ\s\.]*)", RegexOptions.IgnoreCase);
                if (matchNombreEmisorFallback.Success)
                {
                    documento.NombreEmisor = matchNombreEmisorFallback.Groups[1].Value.Trim();
                    _logger.LogInformation("Nombre del emisor (fallback) extraído: {Nombre}", documento.NombreEmisor);
                }
                else
                {
            ...
            {
                // Formato Alexis Montenegro: GIRO: AGENCIA DE ADUANA Y ASESORIAS ELECTRONICA
                var matchGiroEmisor = Regex.Match(texto, @"GIRO:\s*([A-ZÁÉÍÓÚÑ\s]+?)\s+Casa\s+Matriz:", RegexOptions.IgnoreCase);
                if (matchGiroEmisor.Success)
                {
                    giroEmisor = matchGiroEmisor.Groups[1].Value.Trim();
                    _logger.LogInformation("Giro del emisor (Alexis Montenegro) extraído: {Giro}", giroEmisor);
                }
            }
            ...
            {
                // Formato Alexis Montenegro: Casa Matriz: ESMERALDA 940 OF.111-B - VALPARAISO
                var matchDireccionEmisor = Regex.Match(texto, @"Casa\s+Matriz:\s*([A-ZÁÉÍÓÚÑ\s\d\-\.\-]+?)\s+N[°º]", RegexOptions.IgnoreCase);
                if (matchDireccionEmisor.Success)
                {
                    direccionEmisor = matchDireccionEmisor.Groups[1].Value.Trim();
                    _logger.LogInformation("Dirección del emisor (Alexis Montenegro) extraída: {Direccion}", direccionEmisor);
                }
            }
            ...
            {
                // Buscar VALPARAISO en la dirección
                var matchCiudadEmisor = Regex.Match(texto, @"Casa\s+Matriz:\s*[A-ZÁÉÍÓÚÑ\s\d\-\.\-]+?([A-ZÁÉÍÓÚÑ]+)(?:\s+N[°º])", RegexOptions.IgnoreCase);
                if (matchCiudadEmisor.Success)
                {
                    ciudadEmisor = matchCiudadEmisor.Groups[1].Value.Trim();
                    _logger.LogInformation("Ciudad del emisor (Alexis Montenegro) extraída: {Ciudad}", ciudadEmisor);
                }
            }
            ...
            {
                // Formato Ricardo Mewes: Razón Social: VIRUTEX ILKO S.P.A
                var matchNombreReceptor = Regex.Match(texto, @"RECEPTOR.*?Razón Social:\s*([A-ZÁÉÍÓÚÑ\s\.]+?)\s+Rut:", RegexOptions.IgnoreCase | RegexOptions.Singleline);
                if (matchNombreReceptor.Success)
                {
                    nombreReceptor = matchNombreReceptor.Groups[1].Value.Trim();
                    _logger.LogInformation("Nombre del receptor (Ricardo Mewes) extraído: {Nombre}", nombreReceptor);
                }
            }
            else if (esFormatoAlexisMontenegro)
            {
                // Formato Alexis Montenegro: COMERCIAL CASA NOVEDAD LIMITADA
                var matchNombreReceptor = Regex.Match(texto, @"Fecha\s+Trasportiett\s+([A-ZÁÉÍÓÚÑ\s]+?)\s+LAS\s+ORQUIDEAS", RegexOptions.IgnoreCase);
                if (matchNombreReceptor.Success)
                {
                    nombreReceptor = matchNombreReceptor.Groups[1].Value.Trim();
                    _logger.LogInformation("Nombre del receptor (Alexis Montenegro) extraído: {Nombre}", nombreReceptor);
                }
            }
            ...
            {
                // Fallback: buscar después de "Razón Social:" sin restricciones de formato
                var matchNombreReceptorFallback = Regex.Match(texto, @"Razón Social:\s*([A-ZÁÉÍÓÚÑ\s\.]+?)(?:\s+Rut:|$)", RegexOptions.IgnoreCase);
                if (matchNombreReceptorFallback.Success)
                {
                    documento.NombreReceptor = matchNombreReceptorFallback.Groups[1].Value.Trim();
                    _logger.LogInformation("Nombre del receptor (fallback) extraído: {Nombre}", documento.NombreReceptor);
                }
                else
                {
            ...
            {
                // Formato Ricardo Mewes: buscar el segundo Rut: después de RECEPTOR
                var matchRutReceptor = Regex.Match(texto, @"RECEPTOR.*?Razón Social:.*?Rut:\s*(\d{1,2}\.\d{3}\.\d{3}-\d)", RegexOptions.IgnoreCase | RegexOptions.Singleline);
                if (matchRutReceptor.Success)
                {
                    rutReceptor = matchRutReceptor.Groups[1].Value.Trim();
                    _logger.LogInformation("RUT del receptor (Ricardo Mewes) extraído: {Rut}", rutReceptor);
                }
            }
            else if (esFormatoAlexisMontenegro)
            {
                // Formato Alexis Montenegro: buscar el segundo RUT en el texto
                var matches = Regex.Matches(texto, @"(\d{1,2}\.\d{3}\.\d{3}-\d)", RegexOptions.IgnoreCase);
                if (matches.Count >= 2)
                {
                    rutReceptor = matches[1].Groups[1].Value.Trim();
                    _logger.LogInformation("RUT del receptor (Alexis Montenegro) extraído: {Rut}", rutReceptor);
                }
            }
            ...
            {
                // Fallback: buscar el segundo RUT que aparece en el documento
                var matches = Regex.Matches(texto, @"(\d{1,2}\.\d{3}\.\d{3}-\d)", RegexOptions.IgnoreCase);
                if (matches.Count >= 2)
                {
                    documento.RutReceptor = matches[1].Groups[1].Value.Trim();
                    _logger.LogInformation("RUT del receptor (fallback - segundo RUT) extraído: {Rut}", documento.RutReceptor);
                }
                else
                {
            ...
            {
                // Formato Ricardo Mewes: buscar Dirección: después del segundo RUT
                var matchDireccionReceptor = Regex.Match(texto, @"RECEPTOR.*?Razón Social:.*?Rut:\s*\d+\.\d+\.\d+-\d\s*Dirección:\s*([A-ZÁÉÍÓÚÑ\s\d]+?)\s+Giro:", RegexOptions.IgnoreCase | RegexOptions.Singleline);
                if (matchDireccionReceptor.Success)
                {
                    direccionReceptor = matchDireccionReceptor.Groups[1].Value.Trim();
                    _logger.LogInformation("Dirección del receptor (Ricardo Mewes) extraída: {Direccion}", direccionReceptor);
                }
            }
            else if (esFormatoAlexisMontenegro)
            {
                // Formato Alexis Montenegro: LAS ORQUIDEAS IN - COQUIMBO
                var matchDireccionReceptor = Regex.Match(texto, @"LAS\s+ORQUIDEAS\s+IN\s*-\s*([A-ZÁÉÍÓÚÑ\s]+?)\s+Chudut:", RegexOptions.IgnoreCase);
                if (matchDireccionReceptor.Success)
                {
                    direccionReceptor = "LAS ORQUIDEAS IN - " + matchDireccionReceptor.Groups[1].Value.Trim();
                    _logger.LogInformation("Dirección del receptor (Alexis Montenegro) extraída: {Direccion}", direccionReceptor);
                }
            }
            ...
            {
                // Fallback: buscar después de "Dirección:" sin restricciones de formato
                var matchDireccionReceptorFallback = Regex.Match(texto, @"Dirección:\s*([A-ZÁÉÍÓÚÑ\s\d]+?)(?:\s+Giro:|$)", RegexOptions.IgnoreCase);
                if (matchDireccionReceptorFallback.Success)
                {
                    documento.DireccionReceptor = matchDireccionReceptorFallback.Groups[1].Value.Trim();
                    _logger.LogInformation("Dirección del receptor (fallback) extraída: {Direccion}", documento.DireccionReceptor);
                }
            }

            // Extraer giro del receptor (específico para formato Ricardo Mewes)
            string giroReceptor = string.Empty;
            if (esFormatoRicardoMewes)
            {
                // Formato Ricardo Mewes: Giro: FABR.PRODUC.METALICOS
                var matchGiroReceptor = Regex.Match(texto, @"RECEPTOR.*?Dirección:.*?Giro:\s*([A-ZÁÉÍÓÚÑ\s\.]+?)(?:\s+DATOS|\s*$)", RegexOptions.IgnoreCase | RegexOptions.Singleline);
                if (matchGiroReceptor.Success)
                {
                    giroReceptor = matchGiroReceptor.Groups[1].Value.Trim();
                    _logger.LogInformation("Giro del receptor (Ricardo Mewes) extraído: {Giro}", giroReceptor);
                    documento.GiroReceptor = giroReceptor;
                }
                else
                {
                    // Fallback: buscar después de "Giro:" sin restricciones
                    var matchGiroReceptorFallback = Regex.Match(texto, @"Giro:\s*([A-ZÁÉÍÓÚÑ\s\.]+?)(?:\s+DATOS|\s*$)", RegexOptions.IgnoreCase);
                    if (matchGiroReceptorFallback.Success)
                    {
                        documento.GiroReceptor = matchGiroReceptorFallback.Groups[1].Value.Trim();
                        _logger.LogInformation("Giro del receptor (fallback) extraído: {Giro}", documento.GiroReceptor);
                    }
                }
            }
            ...
            {
                // Formato Alexis Montenegro: Chudut: COQUIMBO
                var matchComunaReceptor = Regex.Match(texto, @"Chudut:\s*([A-ZÁÉÍÓÚÑ\s]+?)\s+Comune:", RegexOptions.IgnoreCase);
                if (matchComunaReceptor.Success)
                {
                    comunaReceptor = matchComunaReceptor.Groups[1].Value.Trim();
                    _logger.LogInformation("Comuna del receptor (Alexis Montenegro) extraída: {Comuna}", comunaReceptor);
                }
            }
            ...
            {
                // Formato Alexis Montenegro: Comune: COQUIMBO
                var matchCiudadReceptor = Regex.Match(texto, @"Comune:\s*([A-ZÁÉÍÓÚÑ\s]+?)\s+Origen", RegexOptions.IgnoreCase);
                if (matchCiudadReceptor.Success)
                {
                    ciudadReceptor = matchCiudadReceptor.Groups[1].Value.Trim();
                    _logger.LogInformation("Ciudad del receptor (Alexis Montenegro) extraída: {Ciudad}", ciudadReceptor);
                }
            }
            ...
            {
                // Formato Alexis Montenegro: Trasportiett (puede ser el transportista)
                var matchTransportista = Regex.Match(texto, @"Fecha\s+([A-ZÁÉÍÓÚÑ\s]+?)\s+COMERCIAL", RegexOptions.IgnoreCase);
                if (matchTransportista.Success)
                {
                    transportista = matchTransportista.Groups[1].Value.Trim();
                    _logger.LogInformation("Transportista (Alexis Montenegro) extraído: {Transportista}", transportista);
                }
            }
            ...
            {
                // Formato Ricardo Mewes: Aduana : SAN ANTONIO
                var matchAduana = Regex.Match(texto, @"Aduana\s*:\s*([A-ZÁÉÍÓÚÑ\s]+?)(?:\s+Ref\s*:|$)", RegexOptions.IgnoreCase);
                if (matchAduana.Success)
                {
                    aduana = matchAduana.Groups[1].Value.Trim();
                    _logger.LogInformation("Aduana (Ricardo Mewes) extraída: {Aduana}", aduana);
                }
            }
            ...
            {
                // Formato Ricardo Mewes: Ref : IM15452 (con espacio antes del :)
                var matchReferencia = Regex.Match(texto, @"Ref\s*:\s*([A-ZÁÉÍÓÚÑ\s\d\-\.]+?)(?:\s+\d+|\s*$)", RegexOptions.IgnoreCase);
                if (matchReferencia.Success)
                {
                    referencia = matchReferencia.Groups[1].Value.Trim();
                    _logger.LogInformation("Referencia (Ricardo Mewes) extraída: {Referencia}", referencia);
                }
            }
            ...
            {
                // Formato Ricardo Mewes: Peso Bruto (Kgs.) : 26.270,00
                var matchPeso = Regex.Match(texto, @"Peso\s+Bruto\s*\(Kgs\.\)\s*:\s*([\d\.,]+)", RegexOptions.IgnoreCase);
                if (matchPeso.Success)
                {
                    var pesoStr = matchPeso.Groups[1].Value;
                    // Convertir formato chileno (26.270,00) a formato decimal
                    if (pesoStr.Contains(","))
                    {
                        // Separar miles de decimales
                        var partes = pesoStr.Split(',');
                        var entero = partes[0].Replace(".", ""); // Quitar separadores de miles
                        var decimales = partes.Length > 1 ? partes[1] : "00";
                        pesoStr = $"{entero}.{decimales}";
                    }
                    
                    if (decimal.TryParse(pesoStr, out var pesoValor))
                    {
                        peso = pesoValor;
                        _logger.LogInformation("Peso (Ricardo Mewes) extraído: {Peso}", peso);
                    }
                }
            }
            else if (esFormatoAlexisMontenegro)
            {
                // Formato Alexis Montenegro: buscar peso en el texto (puede estar en diferentes formatos)
                var matchPeso = Regex.Match(texto, @"(\d+\.\d{3},\d{2})", RegexOptions.IgnoreCase);
                if (matchPeso.Success)
                {
                    var pesoStr = matchPeso.Groups[1].Value;
                    // Convertir formato chileno (5.384,03) a formato decimal
                    if (pesoStr.Contains(","))
                    {
                        // Separar miles de decimales
                        var partes = pesoStr.Split(',');
                    ...
                    {
                        peso = pesoValor;
                        _logger.LogInformation("Peso (Alexis Montenegro) extraído: {Peso}", peso);
                    }
                }
            }
            ...
            {
                // Formato Alexis Montenegro: buscar cantidad en el texto (puede estar en diferentes formatos)
                var matchCantidadBultos = Regex.Match(texto, @"(\d+)\s+CAJAS", RegexOptions.IgnoreCase);
                if (matchCantidadBultos.Success)
                {
                    if (int.TryParse(matchCantidadBultos.Groups[1].Value, out var cantidad))
                    {
                        cantidadBultos = cantidad;
                        _logger.LogInformation("Cantidad de bultos (Alexis Montenegro) extraída: {Cantidad}", cantidadBultos);
                    }
                }
            }
            ...
            {
                // Formato Alexis Montenegro: buscar tipo de bulto en el texto
                var matchTipoBulto = Regex.Match(texto, @"(\d+)\s+CAJAS\s+([A-ZÁÉÍÓÚÑ\s]+?)(?:\s+-|\s+PELOTAS|\s*$)", RegexOptions.IgnoreCase);
                if (matchTipoBulto.Success)
                {
                    tipoBulto = $"{matchTipoBulto.Groups[1].Value} CAJAS {matchTipoBulto.Groups[2].Value}".Trim();
                    _logger.LogInformation("Tipo de bulto (Alexis Montenegro) extraído: {TipoBulto}", tipoBulto);
                }
            }
            ...
            {
                // Formato Alexis Montenegro: buscar observaciones en el texto
                var matchObservaciones = Regex.Match(texto, @"Observaciones:\s*([A-ZÁÉÍÓÚÑ\s\d\-\.\-]+?)(?:\s+CAAU|\s*$)", RegexOptions.IgnoreCase);
                if (matchObservaciones.Success)
                {
                    observaciones = matchObservaciones.Groups[1].Value.Trim();
                    _logger.LogInformation("Observaciones (Alexis Montenegro) extraídas: {Observaciones}", observaciones);
                }
            }
            ...
            // Campos específicos del formato Ricardo Mewes
            if (esFormatoRicardoMewes)
            {
                // Extraer destino (dirección de entrega)
                var matchDestino = Regex.Match(texto, @"DESTINO.*?ENTREGA.*?([A-ZÁÉÍÓÚÑ\s\d]+?ENEA)", RegexOptions.IgnoreCase | RegexOptions.Singleline);
                if (matchDestino.Success)
                {
                    documento.Destino = matchDestino.Groups[1].Value.Trim();
                    _logger.LogInformation("Destino (Ricardo Mewes) extraído: {Destino}", documento.Destino);
                }

                // Extraer tipo de traslado como observación adicional
                var matchTipoTraslado = Regex.Match(texto, @"Tipo traslado:\s*([A-ZÁÉÍÓÚÑ\s]+?)(?:\s+DESTINO|$)", RegexOptions.IgnoreCase);
                if (matchTipoTraslado.Success)
                {
                    var tipoTraslado = matchTipoTraslado.Groups[1].Value.Trim();
                    if (string.IsNullOrEmpty(documento.Observaciones))
                    {
                        documento.Observaciones = $"Tipo traslado: {tipoTraslado}";
                    }
                    else
                    {
                        documento.Observaciones += $" | Tipo traslado: {tipoTraslado}";
                    }
                    _logger.LogInformation("Tipo traslado (Ricardo Mewes) extraído: {TipoTraslado}", tipoTraslado);
                }

                // Extraer información de contenedor
                var matchContenedor = Regex.Match(texto, @"(\d+)\s+(CONT\d+)\s+([A-ZÁÉÍÓÚÑ\d\-]+)", RegexOptions.IgnoreCase);
                if (matchContenedor.Success)
                {
                    var cantidad = matchContenedor.Groups[1].Value;
                    var tipoContenedor = matchContenedor.Groups[2].Value;
                    var numeroContenedor = matchContenedor.Groups[3].Value;
                    
                    if (int.TryParse(cantidad, out var cantidadBultosContenedor))
                    {
                        documento.CantidadBultos = cantidadBultosContenedor;
                    }
                    documento.TipoBulto = tipoContenedor;
                    
                    var infoContenedor = $"Contenedor: {numeroContenedor}";
                    if (string.IsNullOrEmpty(documento.Observaciones))
                    {
                        documento.Observaciones = infoContenedor;
                    }
                    else
                    {
                        documento.Observaciones += $" | {infoContenedor}";
                    ...
                    _logger.LogInformation("Información de contenedor (Ricardo Mewes) extraída: {Cantidad} {Tipo} {Numero}", 
                        cantidad, tipoContenedor, numeroContenedor);
                }
            }
            ...
            {
                // Intentar extraer origen del formato Alexis Montenegro
                if (esFormatoAlexisMontenegro)
                {
                    var matchOrigen = Regex.Match(texto, @"Origen\s+([A-ZÁÉÍÓÚÑ\s]+?)\s+MEDLOG", RegexOptions.IgnoreCase);
                    if (matchOrigen.Success)
                    {
                        documento.Origen = matchOrigen.Groups[1].Value.Trim();
                        _logger.LogInformation("Origen (Alexis Montenegro) extraído: {Origen}", documento.Origen);
                    }
                    else
                    {
                        documento.Origen = "N";
        ...
        private void BusquedaFinalCamposCriticos(GuiaDespacho documento, string texto, bool esFormatoJorgeStein, bool esFormatoAlbertoRubio, bool esFormatoRicardoMewes, bool esFormatoAlexisMontenegro)
        {
            ...
            {
                var matchGuiaFinal = Regex.Match(texto, @"N[°º](\d+)", RegexOptions.IgnoreCase);
                if (matchGuiaFinal.Success)
                {
                    documento.NumeroGuia = matchGuiaFinal.Groups[1].Value;
                    _logger.LogInformation("Número de guía extraído (búsqueda final): {Guia}", documento.NumeroGuia);
                }
            }

            // Búsqueda final para RUT del emisor si aún no se encontró
            if (string.IsNullOrEmpty(documento.RutEmisor))
            {
                var matchRutFinal = Regex.Match(texto, @"R\.U\.T\s*\.\s*:\s*(\d{1,2}\.\d{3}\.\d{3}-\d)", RegexOptions.IgnoreCase);
                if (matchRutFinal.Success)
                {
                    documento.RutEmisor = matchRutFinal.Groups[1].Value;
                    _logger.LogInformation("RUT del emisor extraído (búsqueda final): {Rut}", documento.RutEmisor);
                }
            }

            // Búsqueda final para nombre del emisor si aún no se encontró
            if (string.IsNullOrEmpty(documento.NombreEmisor))
            {
                var matchNombreFinal = Regex.Match(texto, @"R\.U\.T\s*\.\s*:\s*\d{1,2}\.\d{3}\.\d{3}-\d\s+([A-ZÁÉÍÓÚÑ\s]+?)\s+GUIA", RegexOptions.IgnoreCase);
                if (matchNombreFinal.Success)
                {
                    documento.NombreEmisor = matchNombreFinal.Groups[1].Value.Trim();
                    _logger.LogInformation("Nombre del emisor extraído (búsqueda final): {Nombre}", documento.NombreEmisor);
                }
            }

            // Búsqueda final para nombre del receptor si aún no se encontró
            if (string.IsNullOrEmpty(documento.NombreReceptor))
            {
                var matchReceptorFinal = Regex.Match(texto, @"Fecha\s+([A-ZÁÉÍÓÚÑ\s]+?)\s+COMERCIAL", RegexOptions.IgnoreCase);
                if (matchReceptorFinal.Success)
                {
                    documento.NombreReceptor = matchReceptorFinal.Groups[1].Value.Trim();
                    _logger.LogInformation("Nombre del receptor extraído (búsqueda final): {Nombre}", documento.NombreReceptor);
                }
            }

            // Búsqueda final para peso si aún no se encontró
            if (!documento.Peso.HasValue)
            {
                var matchPesoFinal = Regex.Match(texto, @"(\d+\.\d{3},\d{2})", RegexOptions.IgnoreCase);
                if (matchPesoFinal.Success)
                {
                    var pesoStr = matchPesoFinal.Groups[1].Value;
                    // Convertir formato chileno (5.384,03) a formato decimal
                    if (pesoStr.Contains(","))
                    {
                        var partes = pesoStr.Split(',');
                    ...
                    if (decimal.TryParse(pesoStr, out var pesoValor))
                    {
                        documento.Peso = pesoValor;
                        _logger.LogInformation("Peso extraído (búsqueda final): {Peso}", documento.Peso);
                    }
                }
            }

            // Búsqueda final para cantidad de bultos si aún no se encontró
            if (!documento.CantidadBultos.HasValue)
            {
                var matchCantidadFinal = Regex.Match(texto, @"(\d+)\s+CAJAS", RegexOptions.IgnoreCase);
                if (matchCantidadFinal.Success)
                {
                    if (int.TryParse(matchCantidadFinal.Groups[1].Value, out var cantidad))
                    {
                        documento.CantidadBultos = cantidad;
                        _logger.LogInformation("Cantidad de bultos extraída (búsqueda final): {Cantidad}", documento.CantidadBultos);
                    }
                }
            }
        }
