    ...
    public class CarnetAduaneroProcessorService : ICarnetAduaneroProcessorService
    {
        ...
        public async Task<CarnetAduaneroData> ProcesarImagenAsync(Bitmap imagen)
        {
            ...
            try
            {
                ...
                var textoOcr = await _ocrService.ExtractTextAsync(imagen);
        ...
        private async Task<string> ExtraerTextoPngAsync(Stream fileStream)
        {
            try
            {
                ...
                {
                    _logger.LogWarning(skiaEx, "SkiaSharp falló, usando Azure Vision directamente");
                    // Fallback: usar Azure Vision directamente sin procesamiento local
        ...
        private async Task<string> ExtraerTextoConServicioHibridoAsync(SKBitmap bitmap)
        {
            try
            {
                ...
                var texto = await _hybridOcrService.ExtractTextAsync(stream);
        ...
        public static async Task<string> ProcessImageWithSkiaSharpAsync(Stream imageStream, string azureVisionKey, string azureVisionEndpoint, ILogger logger)
        {
            try
            {
                ...
                // Verificar y resetear posición del stream
                logger.LogInformation("Stream Length: {Length}, Position: {Position}", imageStream.Length, imageStream.Position);
