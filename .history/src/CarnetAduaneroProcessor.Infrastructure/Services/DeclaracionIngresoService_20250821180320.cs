...
namespace CarnetAduaneroProcessor.Infrastructure.Services
{
    ...
    /// </summary>
        ...
        public DeclaracionIngresoService(ILogger<DeclaracionIngresoService> logger, IConfiguration configuration, IHybridOcrService hybridOcrService, IAiPostProcessorService aiPostProcessor)
        {
            _logger = logger;
        ...
        public async Task<DeclaracionIngreso> ProcesarTextoOcrAsync(string textoOcr)
        {
            ...
            try
            {
                ...
                // Realizar búsqueda final de campos críticos que no se pudieron extraer
                await BusquedaFinalCamposCriticosAsync(declaracion, textoNormalizado);
        ...
        private async Task ExtraerCamposCriticosAsync(DeclaracionIngreso declaracion, string texto)
        {
            await Task.Run(() =>
            {
                ...
                {
                    // Fallback: buscar cualquier patrón de contenedor
                    var tipoBultoFallback = Regex.Match(texto, @"([A-Z]{4}\d{2,3})");
                    if (tipoBultoFallback.Success)
                    {
                        declaracion.TipoBulto = tipoBultoFallback.Groups[1].Value;
                        _logger.LogInformation("Tipo de bulto extraído (fallback): {Bulto}", declaracion.TipoBulto);
                    }
                }
                ...
                // 6.1. Peso bruto alternativo - buscar en la sección de descripción de mercancías
                if (string.IsNullOrEmpty(declaracion.PesoBruto))
                {
                    var pesoBrutoAltMatch = Regex.Match(texto, @"CONT40\s+Cantidad\s+\d+\s+(\d{1,3}(?:\.\d{3})*(?:,\d{2})?)");
                    if (pesoBrutoAltMatch.Success)
                    {
                        declaracion.PesoBruto = pesoBrutoAltMatch.Groups[1].Value;
                        _logger.LogInformation("Peso bruto extraído (alternativo): {Peso}", declaracion.PesoBruto);
                    }
                }
                ...
                // 9.1. Total a pagar alternativo - buscar en "IGUALA PAGAR EN S"
                if (string.IsNullOrEmpty(declaracion.TotalPagar))
                {
                    var totalPagarAltMatch = Regex.Match(texto, @"IGUALA\s+PAGAR\s+EN\s+S[:\s]*(\d{1,3}(?:\.\d{3})*)");
                    if (totalPagarAltMatch.Success)
                    {
                        declaracion.TotalPagar = totalPagarAltMatch.Groups[1].Value;
                        _logger.LogInformation("Total a pagar extraído (alternativo): {Total}", declaracion.TotalPagar);
                    }
                }
                ...
                {
                    // Fallback: buscar después de "WALTER PEREZ SALAS"
                    var nombreImportadorFallback = Regex.Match(texto, @"(WALTER\s+PEREZ\s+SALAS)");
                    if (nombreImportadorFallback.Success)
                    {
                        declaracion.NombreImportador = nombreImportadorFallback.Groups[1].Value.Trim();
                        _logger.LogInformation("Nombre del importador extraído (fallback): {Nombre}", declaracion.NombreImportador);
                    }
                }
                ...
                {
                    // Fallback: buscar cualquier RUT en el texto
                    var rutImportadorFallback = Regex.Match(texto, @"(\d{1,2}\.\d{3}\.\d{3}-\d)");
                    if (rutImportadorFallback.Success)
                    {
                        declaracion.RutImportador = rutImportadorFallback.Groups[1].Value;
                        _logger.LogInformation("RUT del importador extraído (fallback): {Rut}", declaracion.RutImportador);
                    }
                }
                ...
                {
                    // Fallback: buscar después de "DESCRIPCION DE MERCANCIAS"
                    var descripcionFallback = Regex.Match(texto, @"DESCRIPCION\s+DE\s+MERCANCIAS[:\s]*([A-Z0-9\s\-\.]+?)(?=\d{1,3}\s+\d{4}|$)", RegexOptions.Singleline);
                    if (descripcionFallback.Success)
                    {
                        declaracion.DescripcionMercancias = descripcionFallback.Groups[1].Value.Trim();
                        _logger.LogInformation("Descripción de mercancías extraída (fallback): {Descripcion}", declaracion.DescripcionMercancias);
                    }
                }
        ...
        private async Task ExtraerCamposAdicionalesAsync(DeclaracionIngreso declaracion, string texto)
        {
            await Task.Run(() =>
            {
                ...
                {
                    // Fallback: buscar después de "Marviesto" (error de OCR)
                    var manifiestoFallback = Regex.Match(texto, @"Marviesto[:\s]*(\d+)");
                    if (manifiestoFallback.Success)
                    {
                        declaracion.Manifiesto = manifiestoFallback.Groups[1].Value;
                        _logger.LogInformation("Manifiesto extraído (fallback): {Manifiesto}", declaracion.Manifiesto);
                    }
                }
                ...
                // Valor CIF alternativo - buscar en la sección de descripción de mercancías
                if (string.IsNullOrEmpty(declaracion.ValorCif))
                {
                    var valorCifAltMatch = Regex.Match(texto, @"CONT40\s+Cantidad\s+\d+\s+\d{1,3}(?:\.\d{3})*(?:,\d{2})?\s+(\d{1,3}(?:\.\d{3})*(?:,\d{2})?)");
                    if (valorCifAltMatch.Success)
                    {
                        declaracion.ValorCif = valorCifAltMatch.Groups[1].Value;
                        _logger.LogInformation("Valor CIF extraído (alternativo): {Valor}", declaracion.ValorCif);
                    }
                }
            ...
            // Realizar búsqueda final de campos críticos que no se pudieron extraer
            await BusquedaFinalCamposCriticosAsync(declaracion, texto);
        }

        /// <summary>
        /// Búsqueda final de campos críticos que no se pudieron extraer
        /// </summary>
        private async Task BusquedaFinalCamposCriticosAsync(DeclaracionIngreso declaracion, string texto)
        {
            await Task.Run(() =>
            {
                _logger.LogInformation("Iniciando búsqueda final de campos críticos");

                // Búsqueda final para peso bruto si aún no se encontró
                if (string.IsNullOrEmpty(declaracion.PesoBruto))
                {
                    var matchPesoFinal = Regex.Match(texto, @"(\d{1,3}(?:\.\d{3})*(?:,\d{2})?)", RegexOptions.IgnoreCase);
                    if (matchPesoFinal.Success)
                    {
                        declaracion.PesoBruto = matchPesoFinal.Groups[1].Value;
                        _logger.LogInformation("Peso bruto extraído (búsqueda final): {Peso}", declaracion.PesoBruto);
                    }
                }

                // Búsqueda final para total a pagar si aún no se encontró
                if (string.IsNullOrEmpty(declaracion.TotalPagar))
                {
                    var matchTotalFinal = Regex.Match(texto, @"(\d{1,3}(?:\.\d{3})*)", RegexOptions.IgnoreCase);
                    if (matchTotalFinal.Success)
                    {
                        var totalStr = matchTotalFinal.Groups[1].Value;
                        if (decimal.TryParse(totalStr.Replace(".", ""), out var totalValor) && totalValor > 1000000)
                        {
                            declaracion.TotalPagar = totalStr;
                            _logger.LogInformation("Total a pagar extraído (búsqueda final): {Total}", declaracion.TotalPagar);
                        }
                    }
                }

                // Búsqueda final para manifiesto si aún no se encontró
                if (string.IsNullOrEmpty(declaracion.Manifiesto))
                {
                    var matchManifiestoFinal = Regex.Match(texto, @"(\d{6})", RegexOptions.IgnoreCase);
                    if (matchManifiestoFinal.Success)
                    {
                        declaracion.Manifiesto = matchManifiestoFinal.Groups[1].Value;
                        _logger.LogInformation("Manifiesto extraído (búsqueda final): {Manifiesto}", declaracion.Manifiesto);
                    }
                }

                // Búsqueda final para RUT del consignatario si aún no se encontró
                if (string.IsNullOrEmpty(declaracion.RutConsignatario))
                {
                    var matchRutFinal = Regex.Match(texto, @"(\d{1,2}\.\d{3}\.\d{3}-\d)", RegexOptions.IgnoreCase);
                    if (matchRutFinal.Success)
                    {
                        declaracion.RutConsignatario = matchRutFinal.Groups[1].Value;
                        _logger.LogInformation("RUT del consignatario extraído (búsqueda final): {Rut}", declaracion.RutConsignatario);
                    }
                }

                // Búsqueda final para descripción de mercancías si aún no se encontró
                if (string.IsNullOrEmpty(declaracion.DescripcionMercancias))
                {
                    var matchDescripcionFinal = Regex.Match(texto, @"([A-Z0-9\s\-\.]+?)(?=\s+\d{1,3}\s+\d{4})", RegexOptions.IgnoreCase);
                    if (matchDescripcionFinal.Success)
                    {
                        declaracion.DescripcionMercancias = matchDescripcionFinal.Groups[1].Value.Trim();
                        _logger.LogInformation("Descripción de mercancías extraída (búsqueda final): {Descripcion}", declaracion.DescripcionMercancias);
                    }
                }

                _logger.LogInformation("Búsqueda final de campos críticos completada");
            });
        }
        ...
        private async Task<string> ExtraerTextoConOcrAsync(Stream fileStream)
        {
            try
            {
                // Usar SkiaSharp para evitar problemas en Linux
