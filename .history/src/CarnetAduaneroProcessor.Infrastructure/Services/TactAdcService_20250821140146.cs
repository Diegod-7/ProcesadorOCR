...
namespace CarnetAduaneroProcessor.Infrastructure.Services
{
    ...
    public class TactAdcService : ITactAdcService
    {
        ...
        public TactAdcService(ILogger<TactAdcService> logger, IConfiguration configuration, IHybridOcrService hybridOcrService)
        {
            _logger = logger;
        ...
        private async Task ExtraerCamposCriticosAsync(TactAdc documento, string texto)
        {
            ...
            // Extraer número de contenedor (formato: MRSU061268-2, TCLU221686-7, TCLU9522391, CMALJ 650120-0 (FCL), etc.)
            var matchContenedor = Regex.Match(texto, @"Contenedor\s*:\s*([A-Z0-9\s\-\(\)]+)", RegexOptions.IgnoreCase);
            if (!matchContenedor.Success)
            {
                // Fallback: buscar en formato específico del texto OCR
                matchContenedor = Regex.Match(texto, @"Contenedor\s*:\s*([A-Z0-9\s\-\(\)]+?)(?=\r\n|\n|$)", RegexOptions.IgnoreCase);
            }
            if (!matchContenedor.Success)
            {
                // Fallback: buscar en formato estándar (formato: MRSU061268-2, TCLU221686-7, etc.)
                matchContenedor = Regex.Match(texto, @"Contenedor\s+([A-Z]{4}\d{6}-\d)", RegexOptions.IgnoreCase);
            }
            if (!matchContenedor.Success)
            {
                // Fallback: buscar en la tabla de contenedores (formato MSC)
            ...
            {
                documento.NumeroContenedor = matchContenedor.Groups[1].Value.Trim();
        ...
        /// Búsqueda final de campos críticos que no se pudieron extraer
        /// </summary>
        private void BusquedaFinalCamposCriticos(TactAdc documento, string texto)
        {
            // Búsqueda final para número de contenedor si aún no se encontró
            if (string.IsNullOrEmpty(documento.NumeroContenedor))
            {
                var matchContenedorFinal = Regex.Match(texto, @"Contenedor\s*:\s*([A-Z0-9\s\-\(\)]+)", RegexOptions.IgnoreCase);
                if (matchContenedorFinal.Success)
                {
                    documento.NumeroContenedor = matchContenedorFinal.Groups[1].Value.Trim();
                    _logger.LogInformation("Número de contenedor extraído (búsqueda final): {Contenedor}", documento.NumeroContenedor);
                }
                else
                {
                    // Último intento: buscar cualquier cosa que contenga "Contenedor" y el formato esperado
                    var matchContenedorUltimo = Regex.Match(texto, @".*?Contenedor\s*:?\s*([A-Z0-9\s\-\(\)]+)", RegexOptions.IgnoreCase);
                    if (matchContenedorUltimo.Success)
                    {
                        documento.NumeroContenedor = matchContenedorUltimo.Groups[1].Value.Trim();
                        _logger.LogInformation("Número de contenedor extraído (último intento): {Contenedor}", documento.NumeroContenedor);
                    }
                    else
                    {
                        // Búsqueda muy agresiva: buscar cualquier patrón que contenga "Contenedor" y el formato CMALJ
                        var matchContenedorMuyAgresivo = Regex.Match(texto, @".*?Contenedor.*?([A-Z]{4,5}\s+\d{6}-\d\s*\([A-Z]+\))", RegexOptions.IgnoreCase);
                        if (matchContenedorMuyAgresivo.Success)
                        {
                            documento.NumeroContenedor = matchContenedorMuyAgresivo.Groups[1].Value.Trim();
                            _logger.LogInformation("Número de contenedor extraído (muy agresiva): {Contenedor}", documento.NumeroContenedor);
                        }
                        else
                        {
                            // Búsqueda desesperada: buscar cualquier patrón que contenga el formato CMALJ
                            var matchContenedorDesesperado = Regex.Match(texto, @"([A-Z]{4,5}\s+\d{6}-\d\s*\([A-Z]+\))", RegexOptions.IgnoreCase);
                            if (matchContenedorDesesperado.Success)
                            {
                                documento.NumeroContenedor = matchContenedorDesesperado.Groups[1].Value.Trim();
                                _logger.LogInformation("Número de contenedor extraído (búsqueda desesperada): {Contenedor}", documento.NumeroContenedor);
                            }
                        }
                    }
                }
            ...
            // Búsqueda final para número TATC si aún no se encontró
            if (string.IsNullOrEmpty(documento.NumeroTatc))
            {
                var matchTatcFinal = Regex.Match(texto, @"TATC\s*:\s*(\d{16})", RegexOptions.IgnoreCase);
                if (matchTatcFinal.Success)
                {
                    documento.NumeroTatc = matchTatcFinal.Groups[1].Value;
                    _logger.LogInformation("Número TATC extraído (búsqueda final): {Tatc}", documento.NumeroTatc);
                }
                else
                {
                    // Último intento: buscar cualquier número de 16 dígitos después de "TATC"
                    var matchTatcUltimo = Regex.Match(texto, @"TATC.*?(\d{16})", RegexOptions.IgnoreCase);
                    if (matchTatcUltimo.Success)
                    {
                        documento.NumeroTatc = matchTatcUltimo.Groups[1].Value;
                        _logger.LogInformation("Número TATC extraído (último intento): {Tatc}", documento.NumeroTatc);
                    }
                }
            }

            // Búsqueda final para número de sellos si aún no se encontró
            if (string.IsNullOrEmpty(documento.NumeroSellos))
            {
                var matchSellosFinal = Regex.Match(texto, @"Sellos\s*:\s*([A-Z0-9]+)", RegexOptions.IgnoreCase);
                if (matchSellosFinal.Success)
                {
                    documento.NumeroSellos = matchSellosFinal.Groups[1].Value;
                    _logger.LogInformation("Número de sellos extraído (búsqueda final): {Sellos}", documento.NumeroSellos);
                }
                else
                {
                    // Último intento: buscar cualquier patrón que contenga "Sellos" y números
                    var matchSellosUltimo = Regex.Match(texto, @".*?Sellos\s*:?\s*([A-Z0-9]+)", RegexOptions.IgnoreCase);
                    if (matchSellosUltimo.Success)
                    {
                        documento.NumeroSellos = matchSellosUltimo.Groups[1].Value;
                        _logger.LogInformation("Número de sellos extraído (último intento): {Sellos}", documento.NumeroSellos);
                    }
                }
            }
        }
