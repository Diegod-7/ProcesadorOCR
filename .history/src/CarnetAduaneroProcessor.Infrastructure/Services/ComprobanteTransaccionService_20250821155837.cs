...
namespace CarnetAduaneroProcessor.Infrastructure.Services
{
    ...
    public class ComprobanteTransaccionService : IComprobanteTransaccionService
    {
        ...
        public ComprobanteTransaccionService(ILogger<ComprobanteTransaccionService> logger, IConfiguration configuration, IHybridOcrService hybridOcrService, IAiPostProcessorService aiPostProcessor)
        {
            _logger = logger;
        ...
        public async Task<ComprobanteTransaccion> ProcesarTextoOcrAsync(string textoOcr)
        {
            ...
            try
            {
                ...
                // Extraer fechas de manera robusta
                ExtraerFechasRobustas(documento, textoNormalizado);
                ...
                {
                    _logger.LogWarning("Extracción incompleta, intentando post-procesamiento con IA");
                    
                    // Intentar post-procesamiento con IA
                    try
                    {
                        var documentoJson = JsonSerializer.Serialize(documento, new JsonSerializerOptions { WriteIndented = true });
                        var documentoCompletadoJson = await _aiPostProcessor.PostProcesarDocumentoAsync(documentoJson, textoNormalizado);
                        
                        if (!string.IsNullOrEmpty(documentoCompletadoJson) && documentoCompletadoJson != documentoJson)
                        {
                            var documentoCompletado = JsonSerializer.Deserialize<ComprobanteTransaccion>(documentoCompletadoJson);
                            if (documentoCompletado != null)
                            {
                                // Copiar campos completados
                                documento.NumeroFolio = documentoCompletado.NumeroFolio ?? documento.NumeroFolio;
                                documento.TotalPagado = documentoCompletado.TotalPagado > 0 ? documentoCompletado.TotalPagado : documento.TotalPagado;
                                documento.Formulario = documentoCompletado.Formulario ?? documento.Formulario;
                                documento.FechaVencimiento = documentoCompletado.FechaVencimiento ?? documento.FechaVencimiento;
                                documento.MonedaPago = documentoCompletado.MonedaPago ?? documento.MonedaPago;
                                documento.FechaPago = documentoCompletado.FechaPago ?? documento.FechaPago;
                                documento.InstitucionRecaudadora = documentoCompletado.InstitucionRecaudadora ?? documento.InstitucionRecaudadora;
                                documento.IdentificadorTransaccion = documentoCompletado.IdentificadorTransaccion ?? documento.IdentificadorTransaccion;
                                
                                // Revalidar después del post-procesamiento
                                documento.EsValido = ValidarDocumento(documento);
                                if (documento.EsValido)
                                {
                                    documento.Comentarios = "Documento completado exitosamente con post-procesamiento de IA";
                                    _logger.LogInformation("Post-procesamiento con IA exitoso para Comprobante de Transacción: {NumeroFolio}", documento.NumeroFolio);
                                }
                                else
                                {
                                    documento.Comentarios = "No se pudieron extraer todos los campos requeridos del documento de Comprobante de Transacción, incluso con post-procesamiento de IA";
                                    _logger.LogWarning("Extracción incompleta después de IA: {Error}", documento.Comentarios);
                                }
                            }
                        }
                        else
                ...
                {
                    documento.Comentarios = "Documento procesado exitosamente";
        ...
        private async Task ExtraerCamposCriticosAsync(ComprobanteTransaccion documento, string texto)
        {
            ...
            // Buscar el patrón: Folio seguido de números
            var matchFolio = Regex.Match(texto, @"Folio\s+(\d{10})", RegexOptions.IgnoreCase);
            ...
            {
                // Fallback: buscar cualquier número de 10 dígitos después de "Folio"
                var matchFolioFallback = Regex.Match(texto, @"Folio\s+(\d{10})", RegexOptions.IgnoreCase);
                ...
                {
                    // Último fallback: buscar cualquier número de 10 dígitos
                    var matchFolioUltimo = Regex.Match(texto, @"(\d{10})", RegexOptions.IgnoreCase);
            ...
            // Buscar el patrón: Total Pagado seguido de números con puntos
            ...
            {
                // Fallback: buscar cualquier número con formato XX.XXX.XXX
                var matchTotalFallback = Regex.Match(texto, @"(\d{1,2}\.\d{3}\.\d{3})", RegexOptions.IgnoreCase);
                if (matchTotalFallback.Success)
                {
                    var totalStr = matchTotalFallback.Groups[1].Value.Replace(".", "");
                    if (decimal.TryParse(totalStr, out var total))
                    {
                        documento.TotalPagado = total;
                ...
                {
                    // Último fallback: buscar cualquier número de 7 dígitos
                    var matchTotalUltimo = Regex.Match(texto, @"(\d{7})", RegexOptions.IgnoreCase);
                    if (matchTotalUltimo.Success)
                    {
                        var totalStr = matchTotalUltimo.Groups[1].Value;
                        if (decimal.TryParse(totalStr, out var total))
                        {
                            documento.TotalPagado = total;
        ...
        private async Task ExtraerCamposAdicionalesAsync(ComprobanteTransaccion documento, string texto)
        {
            ...
            // Buscar el patrón: Formulario seguido de números
            ...
            {
                // Fallback: buscar cualquier número de 1-2 dígitos después de "Formulario"
                var matchFormularioFallback = Regex.Match(texto, @"Formulario\s+(\d{1,2})", RegexOptions.IgnoreCase);
                ...
                {
                    // Último fallback: buscar cualquier número de 1-2 dígitos
                    var matchFormularioUltimo = Regex.Match(texto, @"(\d{1,2})", RegexOptions.IgnoreCase);
            ...
            // Buscar el patrón: Moneda de Pago seguido de letras
            ...
            {
                // Fallback: buscar cualquier secuencia de 3 letras mayúsculas
                var matchMonedaFallback = Regex.Match(texto, @"(\b[A-Z]{3}\b)", RegexOptions.IgnoreCase);
            ...
            // Buscar el patrón: Institución Recaudadora seguido de letras
            ...
            {
                // Fallback: buscar "BANCO" seguido de letras
                var matchInstitucionFallback = Regex.Match(texto, @"(BANCO\s+[A-Z]+)", RegexOptions.IgnoreCase);
                ...
                {
                    // Último fallback: buscar cualquier secuencia de "BANCO" + letras
                    var matchInstitucionUltimo = Regex.Match(texto, @"(BANCO\s+[A-Z]+)", RegexOptions.IgnoreCase);
            ...
            // Buscar el patrón: Identificador de Transacción seguido de números con guion
            ...
            {
                // Fallback: buscar cualquier secuencia de números-guión-números
                var matchIdentificadorFallback = Regex.Match(texto, @"(\d{8}-\d{8})", RegexOptions.IgnoreCase);
                ...
                {
                    // Último fallback: buscar cualquier secuencia de números-guión-números
                    var matchIdentificadorUltimo = Regex.Match(texto, @"(\d+-\d+)", RegexOptions.IgnoreCase);
                    if (matchIdentificadorUltimo.Success)
                    {
                        documento.IdentificadorTransaccion = matchIdentificadorUltimo.Groups[1].Value;
        ...
        /// <summary>
        /// Extrae fechas del texto OCR de manera robusta
        /// </summary>
        private void ExtraerFechasRobustas(ComprobanteTransaccion documento, string texto)
        {
            // Buscar fechas con formato DD-MM-YYYY
            var fechas = Regex.Matches(texto, @"(\d{2}-\d{2}-\d{4})", RegexOptions.IgnoreCase);
            
            if (fechas.Count >= 2)
            {
                // La primera fecha suele ser la de vencimiento
                if (DateTime.TryParse(fechas[0].Value, out var vencimiento))
                {
                    documento.FechaVencimiento = vencimiento;
                    _logger.LogInformation("Fecha de vencimiento extraída: {Vencimiento}", documento.FechaVencimiento);
                }

                // La segunda fecha suele ser la de pago
                if (DateTime.TryParse(fechas[1].Value, out var pago))
                {
                    documento.FechaPago = pago;
                    _logger.LogInformation("Fecha de pago extraída: {FechaPago}", documento.FechaPago);
                }
            }
            else if (fechas.Count == 1)
            {
                // Si solo hay una fecha, intentar determinar cuál es
                var fecha = DateTime.Parse(fechas[0].Value);
                if (fecha > DateTime.Now)
                {
                    // Fecha futura = vencimiento
                    documento.FechaVencimiento = fecha;
                    _logger.LogInformation("Fecha de vencimiento extraída: {Vencimiento}", documento.FechaVencimiento);
                }
                else
                {
