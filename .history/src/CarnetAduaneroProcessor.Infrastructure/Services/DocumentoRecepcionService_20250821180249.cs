...
namespace CarnetAduaneroProcessor.Infrastructure.Services
{
    ...
    /// </summary>
            public class DocumentoRecepcionService : IDocumentoRecepcionService
        {
            ...
            private readonly IAiPostProcessorService _aiPostProcessor;
            private readonly string _azureVisionKey;
            private readonly string _azureVisionEndpoint;

        public DocumentoRecepcionService(ILogger<DocumentoRecepcionService> logger, IConfiguration configuration, IHybridOcrService hybridOcrService, IAiPostProcessorService aiPostProcessor)
        {
            _logger = logger;
        ...
        private async Task ExtraerCamposCriticosAsync(DocumentoRecepcion documento, string texto)
        {
            await Task.Run(() =>
            {
                ...
                else
                {
                    ...
                    {
                        // Segundo fallback: buscar con patrón más específico del texto OCR
                        var blArmadorFallback2 = Regex.Match(texto, @"BL\s+Armador\s*:?\s*([A-Z0-9\(\)\/\s]+?)(?=\r\n|\n|Consignatario|$)");
                        if (blArmadorFallback2.Success)
                        {
                            documento.BlArmador = blArmadorFallback2.Groups[1].Value.Trim();
                            _logger.LogInformation("BL Armador extraído (fallback 2): {BL}", documento.BlArmador);
                        }
                        else
                        {
                            // Tercer fallback: buscar con patrón más flexible
                            var blArmadorFallback3 = Regex.Match(texto, @"BL\s+Armador\s*:?\s*([^:\r\n]+?)(?=\r\n|\n|Consignatario|$)");
                            if (blArmadorFallback3.Success)
                            {
                                documento.BlArmador = blArmadorFallback3.Groups[1].Value.Trim();
                                _logger.LogInformation("BL Armador extraído (fallback 3): {BL}", documento.BlArmador);
                            }
                        }
                    }
                }
                ...
                else
                {
                    ...
                    {
                        // Segundo fallback: buscar con patrón más específico del texto OCR
                        var naveViajeFallback2 = Regex.Match(texto, @"Nave/Viaje\s*:?\s*([A-Z\s]+/\s*[A-Z0-9]+?)(?=\r\n|\n|Linea|$)");
                        if (naveViajeFallback2.Success)
                        {
                            documento.NaveViaje = naveViajeFallback2.Groups[1].Value.Trim();
                            _logger.LogInformation("Nave/Viaje extraído (fallback 2): {NaveViaje}", documento.NaveViaje);
                        }
                        else
                        {
                            // Tercer fallback: buscar con patrón más flexible
                            var naveViajeFallback3 = Regex.Match(texto, @"Nave/Viaje\s*:?\s*([^:\r\n]+?)(?=\r\n|\n|Linea|$)");
                            if (naveViajeFallback3.Success)
                            {
                                documento.NaveViaje = naveViajeFallback3.Groups[1].Value.Trim();
                                _logger.LogInformation("Nave/Viaje extraído (fallback 3): {NaveViaje}", documento.NaveViaje);
                            }
                            else
                            {
                                // Cuarto fallback: buscar con patrón muy flexible
                                var naveViajeFallback4 = Regex.Match(texto, @"Nave/Viaje\s*:?\s*([^:\r\n]+?)(?=\r\n|\n|$)");
                                if (naveViajeFallback4.Success)
                                {
                                    documento.NaveViaje = naveViajeFallback4.Groups[1].Value.Trim();
                                    _logger.LogInformation("Nave/Viaje extraído (fallback 4): {NaveViaje}", documento.NaveViaje);
                                }
                            }
                        }
                    }
                ...
                // BL Armador específico del texto OCR - búsqueda adicional
                if (string.IsNullOrEmpty(documento.BlArmador))
                {
                    var blArmadorEspecificoMatch = Regex.Match(texto, @"BL\s+Armador\s*:\s*([^:\r\n]+?)(?=\r\n|\n|Consignatario|$)");
                    if (blArmadorEspecificoMatch.Success)
                    {
                        documento.BlArmador = blArmadorEspecificoMatch.Groups[1].Value.Trim();
                        _logger.LogInformation("BL Armador extraído (específico): {BL}", documento.BlArmador);
                    }
                    else
                    {
                        // Búsqueda más agresiva para BL Armador
                        var blArmadorAgresivoMatch = Regex.Match(texto, @"BL\s+Armador\s*:\s*([^:\r\n]+?)(?=\r\n|\n|$)");
                        if (blArmadorAgresivoMatch.Success)
                        {
                            documento.BlArmador = blArmadorAgresivoMatch.Groups[1].Value.Trim();
                            _logger.LogInformation("BL Armador extraído (agresivo): {BL}", documento.BlArmador);
                        }
                    }
                }

                // Nave/Viaje específico del texto OCR - búsqueda adicional
                if (string.IsNullOrEmpty(documento.NaveViaje))
                {
                    var naveViajeEspecificoMatch = Regex.Match(texto, @"Nave/Viaje\s*:\s*([^:\r\n]+?)(?=\r\n|\n|Linea|$)");
                    if (naveViajeEspecificoMatch.Success)
                    {
                        documento.NaveViaje = naveViajeEspecificoMatch.Groups[1].Value.Trim();
                        _logger.LogInformation("Nave/Viaje extraído (específico): {NaveViaje}", documento.NaveViaje);
                    }
                    else
                    {
                        // Búsqueda más agresiva para Nave/Viaje
                        var naveViajeAgresivoMatch = Regex.Match(texto, @"Nave/Viaje\s*:\s*([^:\r\n]+?)(?=\r\n|\n|$)");
                        if (naveViajeAgresivoMatch.Success)
                        {
                            documento.NaveViaje = naveViajeAgresivoMatch.Groups[1].Value.Trim();
                            _logger.LogInformation("Nave/Viaje extraído (agresivo): {NaveViaje}", documento.NaveViaje);
                        }
                    }
                }
                ...
                else
                {
                    ...
                    var cantidadFallback1 = Regex.Match(texto, @"(?:40'|20')\s+[A-Z\s]+?\s+(\d+)\s+[\d\.,]+");
                    if (cantidadFallback1.Success)
                    {
                        documento.Cantidad = cantidadFallback1.Groups[1].Value;
                        _logger.LogInformation("Cantidad extraída (fallback tabla): {Cantidad}", documento.Cantidad);
                    }
                    else
                    {
                        // Fallback 2: buscar después de "CONTENEDOR"
                        var cantidadFallback2 = Regex.Match(texto, @"CONTENEDOR\s+[A-Z\s]+?\s+(\d+)\s+[\d\.,]+");
                        if (cantidadFallback2.Success)
                        {
                            documento.Cantidad = cantidadFallback2.Groups[1].Value;
                            _logger.LogInformation("Cantidad extraída (fallback contenedor): {Cantidad}", documento.Cantidad);
                        }
                        else
                        {
                            // Fallback 3: buscar cualquier número después de "STD" o "NORMAL"
                            var cantidadFallback3 = Regex.Match(texto, @"(?:STD|NORMAL)\s+(\d+)\s+[\d\.,]+");
                            if (cantidadFallback3.Success)
                            {
                                documento.Cantidad = cantidadFallback3.Groups[1].Value;
                                _logger.LogInformation("Cantidad extraída (fallback STD): {Cantidad}", documento.Cantidad);
                            }
                        }
                    }
                }

                // Peso - patrón principal
                ...
                else
                {
                    ...
                    var pesoFallback1 = Regex.Match(texto, @"(?:40'|20')\s+[A-Z\s]+?\s+\d+\s+([\d\.,]+)");
                    if (pesoFallback1.Success)
                    {
                        documento.Peso = pesoFallback1.Groups[1].Value;
                        _logger.LogInformation("Peso extraído (fallback tabla): {Peso}", documento.Peso);
                    }
                    else
                    {
                        // Fallback 2: buscar después de "CONTENEDOR" y cantidad
                        var pesoFallback2 = Regex.Match(texto, @"CONTENEDOR\s+[A-Z\s]+?\s+\d+\s+([\d\.,]+)");
                        if (pesoFallback2.Success)
                        {
                            documento.Peso = pesoFallback2.Groups[1].Value;
                            _logger.LogInformation("Peso extraído (fallback contenedor): {Peso}", documento.Peso);
                        }
                        else
                        {
                            // Fallback 3: buscar cualquier número decimal después de "STD" o "NORMAL" y cantidad
                            var pesoFallback3 = Regex.Match(texto, @"(?:STD|NORMAL)\s+\d+\s+([\d\.,]+)");
                            if (pesoFallback3.Success)
                            {
                                documento.Peso = pesoFallback3.Groups[1].Value;
                                _logger.LogInformation("Peso extraído (fallback STD): {Peso}", documento.Peso);
                            }
                            else
                            {
                                // Fallback 4: búsqueda más agresiva en la tabla
                                var pesoFallback4 = Regex.Match(texto, @"(\d+)\s+([\d\.,]+)\s+[\d\.,]+");
                                if (pesoFallback4.Success)
                                {
                                    // Verificar que el primer número sea 1 (cantidad) y extraer el segundo (peso)
                                    if (pesoFallback4.Groups[1].Value == "1")
                                    {
                                        documento.Peso = pesoFallback4.Groups[2].Value;
                                        _logger.LogInformation("Peso extraído (fallback agresivo): {Peso}", documento.Peso);
                                    }
                                }
                            }
                        }
                ...
                BusquedaFinalCamposCriticos(documento, texto);
        ...
        /// Búsqueda final de campos críticos que no se pudieron extraer
        /// </summary>
            private void BusquedaFinalCamposCriticos(DocumentoRecepcion documento, string texto)
    {
        // Búsqueda final para BL Armador si aún no se encontró
        if (string.IsNullOrEmpty(documento.BlArmador))
        {
            var blArmadorFinalMatch = Regex.Match(texto, @"BL\s+Armador\s*:\s*([^:\r\n]+?)(?=\r\n|\n|Consignatario|$)");
            if (blArmadorFinalMatch.Success)
            {
                documento.BlArmador = blArmadorFinalMatch.Groups[1].Value.Trim();
                _logger.LogInformation("BL Armador extraído (búsqueda final): {BL}", documento.BlArmador);
            }
            else
            {
                // Último intento: buscar cualquier cosa que contenga "BL" y "Armador"
                var blArmadorUltimoMatch = Regex.Match(texto, @".*?BL\s+Armador\s*:\s*([^:\r\n]+?)(?=\r\n|\n|$)");
                if (blArmadorUltimoMatch.Success)
                {
                    documento.BlArmador = blArmadorUltimoMatch.Groups[1].Value.Trim();
                    _logger.LogInformation("BL Armador extraído (último intento): {BL}", documento.BlArmador);
                }
            }
        }

        // Búsqueda final para Nave/Viaje si aún no se encontró
        if (string.IsNullOrEmpty(documento.NaveViaje))
        {
            var naveViajeFinalMatch = Regex.Match(texto, @"Nave/Viaje\s*:\s*([^:\r\n]+?)(?=\r\n|\n|Linea|$)");
            if (naveViajeFinalMatch.Success)
            {
                documento.NaveViaje = naveViajeFinalMatch.Groups[1].Value.Trim();
                _logger.LogInformation("Nave/Viaje extraído (búsqueda final): {NaveViaje}", documento.NaveViaje);
            }
            else
            {
                // Último intento: buscar cualquier cosa que contenga "Nave/Viaje"
                var naveViajeUltimoMatch = Regex.Match(texto, @".*?Nave/Viaje\s*:\s*([^:\r\n]+?)(?=\r\n|\n|$)");
                if (naveViajeUltimoMatch.Success)
                {
                    documento.NaveViaje = naveViajeUltimoMatch.Groups[1].Value.Trim();
                    _logger.LogInformation("Nave/Viaje extraído (último intento): {NaveViaje}", documento.NaveViaje);
                }
            }
        }

        // Búsqueda final para Cantidad si aún no se encontró
        if (string.IsNullOrEmpty(documento.Cantidad))
        {
            // Búsqueda en la tabla de bultos
            var cantidadFinalMatch = Regex.Match(texto, @"(?:40'|20')\s+[A-Z\s]+?\s+(\d+)\s+[\d\.,]+");
            if (cantidadFinalMatch.Success)
            {
                documento.Cantidad = cantidadFinalMatch.Groups[1].Value;
                _logger.LogInformation("Cantidad extraída (búsqueda final): {Cantidad}", documento.Cantidad);
            }
            else
            {
                // Último intento: buscar cualquier número después de "STD" o "NORMAL"
                var cantidadUltimoMatch = Regex.Match(texto, @"(?:STD|NORMAL)\s+(\d+)\s+[\d\.,]+");
                if (cantidadUltimoMatch.Success)
                {
                    documento.Cantidad = cantidadUltimoMatch.Groups[1].Value;
                    _logger.LogInformation("Cantidad extraída (último intento): {Cantidad}", documento.Cantidad);
                }
            }
        }

        // Búsqueda final para Peso si aún no se encontró
        if (string.IsNullOrEmpty(documento.Peso))
        {
            // Búsqueda en la tabla de bultos después de la cantidad
            var pesoFinalMatch = Regex.Match(texto, @"(?:40'|20')\s+[A-Z\s]+?\s+\d+\s+([\d\.,]+)");
            if (pesoFinalMatch.Success)
            {
                documento.Peso = pesoFinalMatch.Groups[1].Value;
                _logger.LogInformation("Peso extraído (búsqueda final): {Peso}", documento.Peso);
            }
            else
            {
                // Último intento: buscar cualquier número decimal después de "STD" o "NORMAL" y cantidad
                var pesoUltimoMatch = Regex.Match(texto, @"(?:STD|NORMAL)\s+\d+\s+([\d\.,]+)");
                if (pesoUltimoMatch.Success)
                {
                    documento.Peso = pesoUltimoMatch.Groups[1].Value;
                    _logger.LogInformation("Peso extraído (último intento): {Peso}", documento.Peso);
                }
                else
                {
                    // Búsqueda muy agresiva: buscar patrón "1 [peso] [volumen]"
                    var pesoAgresivoMatch = Regex.Match(texto, @"1\s+([\d\.,]+)\s+[\d\.,]+");
                    if (pesoAgresivoMatch.Success)
                    {
                        documento.Peso = pesoAgresivoMatch.Groups[1].Value;
                        _logger.LogInformation("Peso extraído (búsqueda agresiva): {Peso}", documento.Peso);
                    }
                }
            }
        }
