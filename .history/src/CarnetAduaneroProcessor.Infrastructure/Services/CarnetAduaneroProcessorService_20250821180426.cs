...
namespace CarnetAduaneroProcessor.Infrastructure.Services
{
    public class CarnetAduaneroProcessorService : ICarnetAduaneroProcessorService
    {
        ...
        public CarnetAduaneroProcessorService(ILogger<CarnetAduaneroProcessorService> logger, IConfiguration configuration, IHybridOcrService ocrService, IAiPostProcessorService aiPostProcessor)
        {
            _logger = logger;
        ...
        public async Task<CarnetAduaneroData> ProcesarTextoOcrAsync(string textoOcr)
        {
            ...
            try
            {
                ...
                if (!resultado.EsValido)
                {
                    ...
                    try
                    {
                        var documentoJson = JsonSerializer.Serialize(resultado, new JsonSerializerOptions { WriteIndented = true });
                        ...
                        {
                            var documentoCompletado = JsonSerializer.Deserialize<CarnetAduaneroData>(documentoCompletadoJson);
                            if (documentoCompletado != null)
                            {
                                // Copiar campos completados por IA
                                if (string.IsNullOrEmpty(resultado.NombreCompleto) && !string.IsNullOrEmpty(documentoCompletado.NombreCompleto))
                                    resultado.NombreCompleto = documentoCompletado.NombreCompleto;
                                
                                if (string.IsNullOrEmpty(resultado.Rut) && !string.IsNullOrEmpty(documentoCompletado.Rut))
                                    resultado.Rut = documentoCompletado.Rut;
                                
                                if (string.IsNullOrEmpty(resultado.NumeroCarne) && !string.IsNullOrEmpty(documentoCompletado.NumeroCarne))
                                    resultado.NumeroCarne = documentoCompletado.NumeroCarne;
                                
                                if (string.IsNullOrEmpty(resultado.FechaEmision) && !string.IsNullOrEmpty(documentoCompletado.FechaEmision))
                                    resultado.FechaEmision = documentoCompletado.FechaEmision;
                                
                                if (string.IsNullOrEmpty(resultado.Resolucion) && !string.IsNullOrEmpty(documentoCompletado.Resolucion))
                                    resultado.Resolucion = documentoCompletado.Resolucion;
                                
                                _logger.LogInformation("IA completó campos faltantes del carné aduanero");
                            }
                        }
                    }
                    catch (Exception ex)
                    {
                        _logger.LogError(ex, "Error en post-procesamiento con IA para carné aduanero");
                    }
                    
                    // Verificar si ahora es válido
                    if (!resultado.EsValido)
                    {
                        resultado.MensajeError = "No se pudieron extraer todos los campos requeridos del carné aduanero, incluso con IA";
                        _logger.LogWarning("Extracción incompleta después de IA: {Error}", resultado.MensajeError);
                    }
                    else
                    {
                        _logger.LogInformation("Procesamiento completado exitosamente con IA para carné: {NumeroCarne}", resultado.NumeroCarne);
                    }
                }
                else
                {
                    _logger.LogInformation("Procesamiento completado exitosamente para carné: {NumeroCarne}", resultado.NumeroCarne);
                }
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        private bool ValidarDocumento(CarnetAduaneroData documento)
        {
            // Verificar que se haya extraído el título
            if (string.IsNullOrWhiteSpace(documento.Titulo))
                return false;

            // Verificar que se haya extraído el nombre completo
            if (string.IsNullOrWhiteSpace(documento.NombreCompleto))
                return false;
            ...
            // Verificar que se haya extraído el número de carné
            if (string.IsNullOrWhiteSpace(documento.NumeroCarne))
                return false;

            // Verificar que se haya extraído la fecha
            if (string.IsNullOrWhiteSpace(documento.FechaEmision))
                return false;

            // Verificar que se haya extraído la resolución
            if (string.IsNullOrWhiteSpace(documento.Resolucion))
                return false;
        ...
        private string LimpiarNombre(string nombre)
        {
            ...
            var nombreLimpio = Regex.Replace(nombre, @"[.\s]+", " ")
                       ...
                       .Replace("neeneenecnriaanneneas", "GONZALEZ") // Corregir texto corrupto específico
                       .Replace("Nombré eeec anannnnnnnes", "GONZALEZ") // Corregir texto corrupto específico
                       .Replace("GONZALEZ..PJ.NO", "GONZALEZ") // Corregir texto corrupto específico
                       .Trim();
            ...
            return string.Join(" ", palabrasFinal);
        }
