...
namespace CarnetAduaneroProcessor.Infrastructure.Services
{
    ...
    public class SeleccionAforoService : ISeleccionAforoService
    {
        ...
        public SeleccionAforoService(ILogger<SeleccionAforoService> logger, IConfiguration configuration, IHybridOcrService hybridOcrService)
        {
            _logger = logger;
        ...
        private async Task ExtraerCamposCriticosAsync(SeleccionAforo documento, string texto)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        // Tercer fallback: buscar con patrón más específico del texto OCR
                        var matchDinFallback3 = Regex.Match(texto, @"Nro\.\s*DIN\s*:?\s*(\d+)", RegexOptions.IgnoreCase);
                        if (matchDinFallback3.Success)
                        {
                            documento.NumeroDin = matchDinFallback3.Groups[1].Value;
                            _logger.LogInformation("Número DIN extraído (fallback 3): {Din}", documento.NumeroDin);
                        }
                        else
                        {
                            // Cuarto fallback: buscar con patrón más flexible
                            var matchDinFallback4 = Regex.Match(texto, @"Nro\.?\s*DIN\s*:?\s*(\d+)", RegexOptions.IgnoreCase);
                            if (matchDinFallback4.Success)
                            {
                                documento.NumeroDin = matchDinFallback4.Groups[1].Value;
                                _logger.LogInformation("Número DIN extraído (fallback 4): {Din}", documento.NumeroDin);
                            }
                            else
                            {
                                // Quinto fallback: buscar cualquier cosa que contenga "DIN" y números
                                var matchDinFallback5 = Regex.Match(texto, @".*?DIN\s*:?\s*(\d+)", RegexOptions.IgnoreCase);
                                if (matchDinFallback5.Success)
                                {
                                    documento.NumeroDin = matchDinFallback5.Groups[1].Value;
                                    _logger.LogInformation("Número DIN extraído (fallback 5): {Din}", documento.NumeroDin);
                                }
                            }
                        }
                    }
            ...
            // Búsqueda específica para el patrón del texto OCR - búsqueda adicional
            if (string.IsNullOrEmpty(documento.NumeroDin))
            {
                var matchDinEspecifico = Regex.Match(texto, @"Nro\.\s*DIN\s*:\s*(\d+)", RegexOptions.IgnoreCase);
                if (matchDinEspecifico.Success)
                {
                    documento.NumeroDin = matchDinEspecifico.Groups[1].Value;
                    _logger.LogInformation("Número DIN extraído (específico): {Din}", documento.NumeroDin);
                }
                else
                {
                    // Búsqueda más agresiva para Nro DIN
                    var matchDinAgresivo = Regex.Match(texto, @"Nro\.\s*DIN\s*:\s*(\d+)", RegexOptions.IgnoreCase);
                    if (matchDinAgresivo.Success)
                    {
                        documento.NumeroDin = matchDinAgresivo.Groups[1].Value;
                        _logger.LogInformation("Número DIN extraído (agresivo): {Din}", documento.NumeroDin);
                    }
                    else
                    {
                        // Búsqueda muy agresiva: buscar cualquier patrón que contenga "DIN" y números
                        var matchDinMuyAgresivo = Regex.Match(texto, @".*?DIN.*?(\d{10})", RegexOptions.IgnoreCase);
                        if (matchDinMuyAgresivo.Success)
                        {
                            documento.NumeroDin = matchDinMuyAgresivo.Groups[1].Value;
                            _logger.LogInformation("Número DIN extraído (muy agresivo): {Din}", documento.NumeroDin);
                        }
                    }
                }
            }
        ...
        /// Búsqueda final de campos críticos que no se pudieron extraer
        /// </summary>
        private void BusquedaFinalCamposCriticos(SeleccionAforo documento, string texto)
        {
            // Búsqueda final para Nro DIN si aún no se encontró
            if (string.IsNullOrEmpty(documento.NumeroDin))
            {
                var matchDinFinal = Regex.Match(texto, @"Nro\.\s*DIN\s*:\s*(\d+)", RegexOptions.IgnoreCase);
                if (matchDinFinal.Success)
                {
                    documento.NumeroDin = matchDinFinal.Groups[1].Value;
                    _logger.LogInformation("Número DIN extraído (búsqueda final): {Din}", documento.NumeroDin);
                }
                else
                {
                    // Último intento: buscar cualquier cosa que contenga "DIN" y números
                    var matchDinUltimo = Regex.Match(texto, @".*?DIN\s*:?\s*(\d+)", RegexOptions.IgnoreCase);
                    if (matchDinUltimo.Success)
                    {
                        documento.NumeroDin = matchDinUltimo.Groups[1].Value;
                        _logger.LogInformation("Número DIN extraído (último intento): {Din}", documento.NumeroDin);
                    }
                    else
                    {
                        // Búsqueda muy agresiva: buscar cualquier patrón que contenga "DIN" y números de 10 dígitos
                        var matchDinMuyAgresivo = Regex.Match(texto, @".*?DIN.*?(\d{10})", RegexOptions.IgnoreCase);
                        if (matchDinMuyAgresivo.Success)
                        {
                            documento.NumeroDin = matchDinMuyAgresivo.Groups[1].Value;
                            _logger.LogInformation("Número DIN extraído (búsqueda muy agresiva): {Din}", documento.NumeroDin);
                        }
                        else
                        {
                            // Búsqueda desesperada: buscar cualquier número de 10 dígitos después de "DIN"
                            var matchDinDesesperado = Regex.Match(texto, @"DIN.*?(\d{10})", RegexOptions.IgnoreCase);
                            if (matchDinDesesperado.Success)
                            {
                                documento.NumeroDin = matchDinDesesperado.Groups[1].Value;
                                _logger.LogInformation("Número DIN extraído (búsqueda desesperada): {Din}", documento.NumeroDin);
                            }
                        }
                    }
                }
