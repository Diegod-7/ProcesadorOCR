...
namespace CarnetAduaneroProcessor.API.Controllers
{
    ...
    public class DocumentoRecepcionController : ControllerBase
    {
        ...
        public async Task<ActionResult<object>> ProcesarDocumentoRecepcion(IFormFile file)
        {
            try
            {
                _logger.LogInformation("Procesando Documento de Recepción con IA desde archivo PNG: {FileName}", file?.FileName);
                ...
                {
                    return BadRequest(new { Status = false, Message = "El archivo no puede exceder 20MB" });
                }
                ...
                if (aiService == null)
                {
                    ...
                    {
                        Status = true,
                        Message = "Documento de Recepción procesado con método tradicional",
                        Data = documento,
                        Metodo = "OCR Tradicional",
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        {
                            Status = true,
                            Message = "Documento de Recepción procesado con método tradicional (fallback)",
                            Data = documentoFallback,
                            Metodo = "OCR Tradicional (Fallback)",
                            ArchivoProcesado = file.FileName
                        });
                    }
                }
