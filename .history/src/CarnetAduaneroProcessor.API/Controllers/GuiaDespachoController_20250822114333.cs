...
namespace CarnetAduaneroProcessor.API.Controllers
{
    ...
    public class GuiaDespachoController : ControllerBase
    {
        ...
        /// <summary>
        /// Sube y procesa un archivo PNG de Guía de Despacho Electrónica usando IA directamente
        /// </summary>
        ...
        public async Task<ActionResult<GuiaDespacho>> ProcesarPng(IFormFile file)
        {
            try
            {
                _logger.LogInformation("Iniciando procesamiento de archivo PNG de Guía de Despacho con IA: {FileName}", file?.FileName);
                ...
                // Convertir JSON a GuiaDespacho
                try
                {
                    var guiaDespacho = JsonSerializer.Deserialize<GuiaDespacho>(jsonExtraido);
                    if (guiaDespacho != null)
                    {
                        // Asignar campos adicionales
                        guiaDespacho.TextoExtraido = jsonExtraido;
                        guiaDespacho.NombreArchivo = file.FileName;
                        guiaDespacho.MetodoExtraccion = "IA (Gemma 3: 4B)";
                        guiaDespacho.FechaProcesamiento = DateTime.UtcNow;

                        _logger.LogInformation("Guía de Despacho procesada exitosamente con IA: {NumeroGuia}", guiaDespacho.NumeroGuia);
                    ...
                    else
                    {
                        ...
                        try
                        {
                            var guiaDespachoFallback = await _guiaDespachoService.ExtraerDatosAsync(tempPath);
                            return CreatedAtAction(nameof(ObtenerPorId), new { id = guiaDespachoFallback.Id }, guiaDespachoFallback);
                        }
                        finally
                        {
                            if (System.IO.File.Exists(tempPath))
