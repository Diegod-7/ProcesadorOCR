...
namespace CarnetAduaneroProcessor.API.Controllers
{
    ...
    public class SeleccionAforoController : ControllerBase
    {
        ...
        /// <summary>
        /// Sube y procesa un archivo PNG de Selección de Aforo usando IA directamente
        /// </summary>
        ...
        public async Task<ActionResult<SeleccionAforo>> ProcesarPng(IFormFile file)
        {
            try
            {
                _logger.LogInformation("Iniciando procesamiento de archivo PNG de Selección de Aforo con IA: {FileName}", file?.FileName);
                ...
                // Convertir JSON a SeleccionAforo
                try
                {
                    var seleccionAforo = JsonSerializer.Deserialize<SeleccionAforo>(jsonExtraido);
                    if (seleccionAforo != null)
                    {
                        // Asignar campos adicionales
                        seleccionAforo.TextoExtraido = jsonExtraido;
                        seleccionAforo.NombreArchivo = file.FileName;
                        seleccionAforo.MetodoExtraccion = "IA (Gemma 3: 4B)";
                        seleccionAforo.FechaProcesamiento = DateTime.UtcNow;

                        _logger.LogInformation("Selección de Aforo procesada exitosamente con IA: {NumeroDin}", seleccionAforo.NumeroDin);
                    ...
                    else
                    {
                        ...
                        try
                        {
                            var seleccionAforoFallback = await _seleccionAforoService.ExtraerDatosAsync(tempPath);
                            return CreatedAtAction(nameof(ObtenerPorId), new { id = seleccionAforoFallback.Id }, seleccionAforoFallback);
                        }
                        finally
                        {
                            if (System.IO.File.Exists(tempPath))
