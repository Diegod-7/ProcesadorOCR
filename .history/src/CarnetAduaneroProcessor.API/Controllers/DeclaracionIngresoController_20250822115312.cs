...
namespace CarnetAduaneroProcessor.API.Controllers
{
    ...
    public class DeclaracionIngresoController : ControllerBase
    {
            ...
            ILogger<DeclaracionIngresoController> logger)
        {
         ...
         /// <summary>
        /// Procesa 1 o 2 archivos PNG de Declaración de Ingreso usando IA directamente
        /// </summary>
        ...
        public async Task<ActionResult<object>> ProcesarDeclaracionIngreso(IFormFileCollection files)
        {
            try
            {
                _logger.LogInformation("Procesando Declaración de Ingreso con IA desde {Count} archivos PNG", files?.Count ?? 0);
                ...
                foreach (var file in files)
                {
                    ...
                    {
                        errores.Add($"Archivo {file.FileName}: No puede exceder 20MB");
                        continue;
                    }
                    ...
                    try
                    {
                        DeclaracionIngreso declaracion;
                        ...
                        if (aiService != null)
                        {
                            ...
                            try
                            {
                                declaracion = JsonSerializer.Deserialize<DeclaracionIngreso>(jsonExtraido);
                                if (declaracion != null)
                                {
                                    // Asignar campos adicionales
                                    declaracion.TextoExtraido = jsonExtraido;
                                    declaracion.NombreArchivo = file.FileName;
                                    declaracion.MetodoExtraccion = "IA (Gemma 3: 4B)";
                                    declaracion.FechaCreacion = DateTime.UtcNow;
                                    declaracion.FechaModificacion = DateTime.UtcNow;
                                }
                                else
                                {
                            ...
                            catch (JsonException)
                            {
                                ...
                                await stream.CopyToAsync(fallbackMemoryStream);
                                var fileBytes = fallbackMemoryStream.ToArray();
                                declaracion = await _declaracionIngresoService.ExtraerDatosAsync(fileBytes, file.FileName);
                                declaracion.MetodoExtraccion = "OCR Tradicional (Fallback)";
                                metodoUsado = "OCR Tradicional (Fallback)";
                            }
                        }
                        else
                        {
                            ...
                            declaracion = await _declaracionIngresoService.ExtraerDatosAsync(fileBytes, file.FileName);
                            declaracion.MetodoExtraccion = "OCR Tradicional";
                        }

                        declaraciones.Add(declaracion);
                        archivosProcesados.Add(file.FileName);

                        _logger.LogInformation("Archivo PNG procesado con {Metodo}: {FileName}", declaracion.MetodoExtraccion, file.FileName);
                    }
                    catch (Exception ex)
                    {
