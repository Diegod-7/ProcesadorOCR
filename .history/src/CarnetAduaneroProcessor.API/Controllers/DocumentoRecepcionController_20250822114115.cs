...
namespace CarnetAduaneroProcessor.API.Controllers
{
    ...
    public class DocumentoRecepcionController : ControllerBase
    {
        ...
        /// <summary>
        /// Procesa un archivo PNG de Documento de Recepción usando IA directamente
        /// </summary>
        /// <param name="file">Archivo PNG a procesar</param>
        /// <returns>Documento de recepción procesado con todos los campos usando IA</returns>
        [HttpPost("procesar")]
        public async Task<ActionResult<object>> ProcesarDocumentoRecepcion(IFormFile file)
        {
            try
            {
                _logger.LogInformation("Procesando Documento de Recepción con IA desde archivo: {FileName}", file?.FileName);
                ...
                {
                    return BadRequest(new { Status = false, Message = "Solo se permiten archivos de imagen válidos" });
                }

                // Validar tamaño del archivo (máximo 20MB para IA)
                if (file.Length > 20 * 1024 * 1024)
                {
                    return BadRequest(new { Status = false, Message = "El archivo es demasiado grande. Máximo 20MB permitido para IA" });
                }
                ...
                // Convertir JSON a DocumentoRecepcion
                try
                {
                    var documento = JsonSerializer.Deserialize<DocumentoRecepcion>(jsonExtraido);
                    if (documento != null)
                    {
                        ...
                        _logger.LogInformation("Documento de Recepción procesado exitosamente con IA: {NumeroDocumento}", documento.NumeroDocumento);

                        return Ok(new
                        {
                            Status = true,
                            Message = "Documento de Recepción procesado correctamente con IA",
                            Data = documento,
                            CamposExtraidos = camposExtraidos,
                            ArchivoProcesado = file.FileName,
                            TamanioArchivo = file.Length,
                            TipoArchivo = file.ContentType,
                            MetodoExtraccion = "IA (Gemma 3: 4B)"
                        });
                    }
                    else
                    {
                        ...
                        using var stream = file.OpenReadStream();
                        var documentoFallback = await _documentoRecepcionService.ExtraerDatosAsync(stream, file.FileName);
                        
                        var camposExtraidos = !string.IsNullOrEmpty(documentoFallback.NumeroDocumento) ||
                                            !string.IsNullOrEmpty(documentoFallback.NumeroManifiesto) ||
                                            !string.IsNullOrEmpty(documentoFallback.BlArmador);

                        return Ok(new
                        {
                            Status = true,
                            Message = "Documento de Recepción procesado correctamente (fallback)",
                            Data = documentoFallback,
                            CamposExtraidos = camposExtraidos,
                            ArchivoProcesado = file.FileName,
                            TamanioArchivo = file.Length,
                            TipoArchivo = file.ContentType,
            ...
            {
                _logger.LogError(ex, "Error procesando Documento de Recepción con IA: {FileName}", file?.FileName);
