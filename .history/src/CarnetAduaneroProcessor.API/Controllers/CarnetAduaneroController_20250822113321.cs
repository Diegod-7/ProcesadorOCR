        ...
        public async Task<ActionResult<CarnetAduaneroData>> ProcesarPng(IFormFile file)
        {
            try
            {
                ...
                try
                {
                    ...
                    if (carnetData != null)
                    {
                        // Asignar el JSON extraído al campo TextoExtraido
                        carnetData.TextoExtraido = jsonExtraido;
                        carnetData.NombreArchivo = file.FileName;
                        carnetData.MetodoExtraccion = "IA (Gemma 3: 4B)";
                        carnetData.FechaProcesamiento = DateTime.UtcNow;
